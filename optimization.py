...
# def func(x, y):
#     return x**3 - y**3 + 3 * x ^ 2 * y


# def grad(x, y):
#     return np.array([[3 * x**2 + 6 * x * y], [-3 * y**2 + 3 * x ^ 2]])


# def AdaCon(r, v, value, gradient, learning_rate, p, a):
#     r_total = []
#     v_total = []
#     concord = np.sum(list(map(lambda x: np.sign(x), gra))) / len(gra)
#     for d in range(len(gra)):
#         r = pr + (1 - p) * gra[d] * gra[d]
#         v = a * learning_rate - concord * learning_rate * gra[d] / np.sqrt(r)
#         r_total.append(r)
#         v_total.append(v)
#     return r_total, v_total


# def train(value, timestep):
#     r = np.array([[0], [0]])
#     v = value
#     for i in range(timestep):
#         g = grad(x, y)
#         r_, v_ = AdaCon(r, v, value, g, learning_rate, 0.99, 0.99)
#         r += r_
#         v += v_
#         return v
